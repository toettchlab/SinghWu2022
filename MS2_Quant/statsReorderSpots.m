function [statslist]=statsReorderSpots(FullStats)
positionlist = [];
statslist = [];
...
for i =1:nFiles
   ...
   tmpSliceA = tmpSliceA';
   tmpX = tmpSliceA(1:5:end);
   tmpY = tmpSliceA(2:5:end);
   tmpIn = tmpSliceA(3:5:end);
   tmpOffsetIn = tmpSliceA(4:5:end);
   tmpInStd=tmpSliceA(5:5:end);
  
   fintmp = [tmpX tmpY repmat(i,[numel(FullStats{i}),1])];
   fintmp1 = [tmpX tmpY tmpIn tmpOffsetIn tmpInStd repmat(i,[numel(FullStats{i}),1])];
