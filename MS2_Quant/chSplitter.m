function  [imgGreenCh,imgRedCh]=chSplitter(myDataFolder,nColor)
...
[imgBkgSubs]=offsetSubs(myImageData,nColor);
...
%         saveTiff((Img{kColor}), [newFolder{kColor},sprintf([ 'file%s','.tif'],fileStr{kColor})]);
%         saveTiff(int16(myFFCorrImage(:,:,:,iFiles)), [OutputFolder,sprintf([ 'file_t%05d','.tif'],iFiles)]);
%   
    end 
end
