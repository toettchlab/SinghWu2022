function [myImg,bw3,Stats]=segSpotZoom5_stdDev_IntSum_smoothn_histSig(imgfile,bw2)

ctrl_tmp = imgfile;
spotOffset=1000;
% Ig=imgaussfilt(ctrl_tmp,0.8);
% % Ig=smoothn(Ig,'robust');
...
for k = 1 : numObj
       ctrl_stats(k).offsetMeanInt = mean(double(ctrl_stats(k).PixelValues((ctrl_stats(k).PixelValues)>spotOffset &(ctrl_stats(k).PixelValues)<55000)));
