function  [maxProjFFBkgGreenCh,maxProjFFBkgRedCh]=maxProjMatFFBkkCorr2ColchSplitter(myDataFolder,nColor,DC,sig)
...
%             maxProjFFBkgRedCh(:,:,iFiles)=double((double(max(imgRedCh(:,:,:,iFiles),[],3))-double(DC)).*Gain_red);
            maxProjFFBkgGreenCh(:,:,iFiles)=double(imflatfield((double(max(imgGreenCh(:,:,:,iFiles),[],3))-double(DC)),sig));
            maxProjFFBkgRedCh(:,:,iFiles)=double(imflatfield((double(max(imgRedCh(:,:,:,iFiles),[],3))-double(DC)),sig));
